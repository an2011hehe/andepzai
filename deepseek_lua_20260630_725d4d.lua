...
local function DeepScanPlayerTeam(plr)
    if TeamCache[plr] ~= nil then
    ...
    pcall(function()
        if plr:FindFirstChild("Team") then
            ...
            if type(teamObj) == "string" then
                teamIdentifier = teamObj:lower()
            elseif typeof(teamObj) == "Instance" then
                local teamName = teamObj.Name or tostring(teamObj)
                teamIdentifier = teamName:lower()
            end
        end
    end)
    ...
    pcall(function()
        ...
        if attr then
            if type(attr) == "string" then teamIdentifier = attr:lower()
            else teamIdentifier = attr end
        end
    end)
    if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
    ...
    if plr.Character then
        ...
        pcall(function()
            ...
            if teamFolder then
                ...
                elseif teamFolder:IsA("ObjectValue") and teamFolder.Value then
                    teamIdentifier = tostring(teamFolder.Value):lower()
                end
                if teamIdentifier and type(teamIdentifier) == "string" then
                    teamIdentifier = teamIdentifier:lower()
                end
            end
        end)
        if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
        ...
        pcall(function()
            ...
            if bodyColors then
                if bodyColors.TorsoColor3 then
        ...
        pcall(function()
            ...
            if head then
                ...
                if not teamIdentifier then
                    ...
                    if mesh and mesh.VertexColor then
                        teamIdentifier = mesh.VertexColor
                    end
                end
            end
        end)
        ...
        pcall(function()
            ...
            if shirt and shirt.ShirtTemplate then
                if shirt.ShirtTemplate:lower():find("red") then teamIdentifier = "red"
                elseif shirt.ShirtTemplate:lower():find("blue") then teamIdentifier = "blue"
                elseif shirt.ShirtTemplate:lower():find("green") then teamIdentifier = "green"
                elseif shirt.ShirtTemplate:lower():find("yellow") then teamIdentifier = "yellow" end
            end
        end)
        if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
        ...
        pcall(function()
            ...
            if humanoid and humanoid.DisplayName then
                local dn = humanoid.DisplayName:lower()
                if dn:find("red") then teamIdentifier = "red"
                elseif dn:find("blue") then teamIdentifier = "blue"
                elseif dn:find("green") then teamIdentifier = "green"
                elseif dn:find("yellow") then teamIdentifier = "yellow" end
            end
        end)
        if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
    end
    ...
    pcall(function()
        local folder = plr:FindFirstChild("PlayerGui")
        if folder then
            local teamGui = folder:FindFirstChild("TeamIndicator") or folder:FindFirstChild("TeamGUI")
            if teamGui and teamGui:FindFirstChildWhichIsA("TextLabel") then
                teamIdentifier = teamGui:FindFirstChildWhichIsA("TextLabel").Text:lower()
            end
        end
    end)
    if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
...
local function ForceRefreshAllTeams()
    local newCache = {}
    for _, plr in ipairs(Players:GetPlayers()) do
        newCache[plr] = DeepScanPlayerTeam(plr)
    end
    TeamCache = newCache
end
ForceRefreshAllTeams()

Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function()
        task.wait(1)
        TeamCache[plr] = DeepScanPlayerTeam(plr)
        ForceRefreshAllTeams()
    end)
end)
...
AimbotConnection = RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if getgenv().AimbotTarget then
        if not IsTargetValid(getgenv().AimbotTarget) then
            getgenv().AimbotTarget = nil
        end
    end

    if not getgenv().AimbotTarget then
