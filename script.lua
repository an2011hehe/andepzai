...
local function ScanPlayerTeam(plr)
    if IsFreeForAll() then
        TeamCache[plr] = "__FFA_NOTEAM__"
        return "__FFA_NOTEAM__"
    end
    ...
    pcall(function()
        for _, child in ipairs(plr:GetChildren()) do
            if child:IsA("ObjectValue") and string.find(child.Name:lower(), "team") then
            ...
            elseif child:IsA("StringValue") and string.find(child.Name:lower(), "team") then
                teamIdentifier = child.Value
                break
            elseif child:IsA("IntValue") and string.find(child.Name:lower(), "team") then
                teamIdentifier = tostring(child.Value)
                break
            end
        end
    ...
    if plr.Character then
        ...
        pcall(function()
            ...
            if teamFolder then
                ...
                elseif teamFolder:IsA("Folder") or teamFolder:IsA("Model") then
                    teamIdentifier = teamFolder.Name
                elseif teamFolder:IsA("ObjectValue") and teamFolder.Value then
                    teamIdentifier = tostring(teamFolder.Value)
                end
            end
        end)
        if teamIdentifier then TeamCache[plr] = teamIdentifier return teamIdentifier end
        ...
        pcall(function()
            for _, child in ipairs(char:GetChildren()) do
                if child:IsA("Folder") and string.find(child.Name:lower(), "team") then
                    teamIdentifier = child.Name
                    break
                end
                if child:IsA("Model") and string.find(child.Name:lower(), "team") then
                    teamIdentifier = child.Name
                    break
                end
    ...
    if teamIdentifier == nil then
        teamIdentifier = "__UNKNOWN_" .. plr.UserId .. "__"
    end

    TeamCache[plr] = teamIdentifier
...
local function IsPlayerEnemy(plr)
    ...
    if myTeam == "__UNKNOWN_" .. LocalPlayer.UserId .. "__" or theirTeam == "__UNKNOWN_" .. plr.UserId .. "__" then
        return true
    end
    ...
    if typeof(myTeam) == "Color3" and typeof(theirTeam) == "Color3" then
        ...
        local rDiff = math.abs(myTeam.R - theirTeam.R)
        local gDiff = math.abs(myTeam.G - theirTeam.G)
        local bDiff = math.abs(myTeam.B - theirTeam.B)
        if rDiff < tolerance and gDiff < tolerance and bDiff < tolerance then
            return false
        end
        return true
    end
