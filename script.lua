    ...
    if teamData == nil and plr.Character ~= nil then
        ...
        if teamData == nil then
            pcall(function()
                ...
                if shirt ~= nil and shirt.ShirtTemplate ~= nil then
                    ...
                    if template:find("red") then teamData = "red"
                    elseif template:find("blue") then teamData = "blue"
                    elseif template:find("green") then teamData = "green"
                    elseif template:find("yellow") then teamData = "yellow" end
                end
            end)
        end
        ...
        if teamData == nil then
            pcall(function()
                ...
                if humanoid ~= nil and humanoid.DisplayName ~= nil then
                    local displayName = humanoid.DisplayName:lower()
                    if displayName:find("red") then teamData = "red"
                    elseif displayName:find("blue") then teamData = "blue"
                    elseif displayName:find("green") then teamData = "green"
                    elseif displayName:find("yellow") then teamData = "yellow" end
                end
            end)
        end
    end
...
AimbotConnection = RunService.RenderStepped:Connect(function(deltaTime)
    ...
    local desiredDirection = (headPosition - cameraPosition).Unit
    local currentDirection = Camera.CFrame.LookVector
    local smoothnessFactor = math.clamp(getgenv().AimbotSmoothness, 0.1, 1.0)
    local interpolationAlpha = math.min(smoothnessFactor * deltaTime * 60, 1)
